...
plot_df = pd.read_csv("Figure12_e.tsv", sep="\t")

category_order = sorted(plot_df["category"].unique())
...
if "n_samples" in plot_df.columns:
    sample_map = dict(zip(
        plot_df[plot_df["Source"] == "Euk"]["category"],
        plot_df[plot_df["Source"] == "Euk"]["n_samples"]
    ))
    xtick_labels = [f"{cat} ({sample_map.get(cat, 0)})" for cat in category_order]
else:
